...
if($kategori == 1){
	...
	$sql = 'select id,nama,"",harga,"J" as status from bop_tetap_standar where id_tetap_standar  = '.$items;
        $query = mysqli_query($koneksi, $sql);
     while ($row = mysqli_fetch_array($query))
        {
...
// echo $sql;
  if(mysqli_query($koneksi, $sql)){
      ...
      echo "<script type='text/javascript'>location.replace('hpp_actual.php?id=".$id_pesanan."');</script>";
